# CTC-007: Specify target markers, carry arcs, and local project model

## Research target

Act as the specification architect for CTC-007, Add target markers, carry arcs,
and local project model. Produce an implementation-ready technical
specification for critical review. Do not write full implementation code.
...
Use current primary sources only where necessary for browser local-file APIs,
JSON security, geographic circle/range-ring math, or accessibility facts.
Clearly separate sourced facts from project design recommendations.
...
- Current repository state: clean synchronized `main` at repository-memory
  commit `cb640e1741deed7b9f71c6037d9c3d1732049424`; CTC-006 merged as
  `52395eaf9a5e26ff7b22b46471faae966fbed3af`.
- CTC-007 is `1. Spec Drafting (Gemini)`.
...
- CTC-006 supplies dependency-free native React SVG selected-hole rendering,
  stable `hole.source.sourceKey` identity, local equirectangular
  projection/inverse projection, Haversine measurement, and pointer/keyboard
...
CTC-007 owns user-authored per-hole target markers, carry-distance settings,
carry-arc rendering, and a versioned local project JSON import/export flow.

- Preserve `NormalizationResult`, `NormalizedHole`, `NormalizedFeature`,
  `Geometry`, `SourceRecord`, and `SourceMetadata` as immutable OSM-derived
  evidence. Do not mutate, heal, replace, or trust imported copies of that
  evidence.
- Persisted hole identity must use `hole.source.sourceKey`; never persist array
  indexes or rely on hole numbers being unique.
- Keep user-authored project data separate from normalized OSM source data.
- Use CTC-006 projection, inverse-projection, distance constants, viewBox,
  clamp behavior, and selected-hole SVG interaction contract unless a narrow
  reviewed extension is necessary.
- Follow `docs/tile-provider-strategy.md`: blank/vector-only MVP, no public OSM
  ...
  attribution whenever OSM-derived geometry is rendered or included in source
  data.
- Prefer existing dependencies and browser APIs. Any new production dependency
  requires explicit reviewed justification and remains unapproved by default.
- Local JSON files stay on the user's device. Do not add network upload,
  accounts, collaboration, server storage, executable content, HTML
  interpretation, or trust in imported source metadata.
- Do not add IndexedDB or localStorage project persistence unless explicitly
  justified and kept separate from CTC-019. Conservative default is memory-only
  project state plus explicit local-file import/export.
- CTC-019 owns durable Overpass cache, refresh, rate limiting, Retry-After,
  backoff, request identity, and durable OSM source cache policy.
- Keep CTC-008 and later notes/layout/PDF/export behavior out of scope.
  CTC-007's local JSON project export is not PDF or rendered-output export.
- Use only explicitly synthetic invented fixtures. Automated tests must remain
  network-isolated.
...
Resolve each question explicitly and against the embedded contracts:

1. Define a strict versioned local project JSON schema. Separate project
   metadata, stable hole-keyed user-authored targets, and carry settings from
   immutable normalized OSM geometry/source evidence. State exactly what source
   identity or provenance references, if any, may be exported.
2. Define strict import validation and safe parsing. Resolve schema version,
   migration policy, maximum file size, JSON root/type checks, unknown fields,
   required/optional fields, finite/range checks, duplicate IDs/hole keys,
   duplicate object keys if relevant, prototype-pollution keys, useful error
   paths/messages, and all-or-nothing versus partial import.
3. Define export behavior: filename, MIME type, deterministic serialization,
   download/browser API lifecycle, source attribution/evidence handling, and
   whether exports contain only user-authored project state or a reviewed
   source reference block.
4. Define the project-state lifecycle. Resolve memory-only versus browser
   persistence, reset/replacement behavior, import conflicts, dirty-state
   behavior, and what happens when loaded normalized course/hole identities do
   not match imported project data. Do not absorb CTC-019.
5. Define target-marker data and identity: stable target IDs, labels/names,
   geographic coordinates, optional metadata, ordering, per-hole ownership,
   coordinate precision, and bounds. Resolve whether targets are free
   geographic coordinates, feature-linked coordinates, or both. Conservative
   default is free geographic coordinates with no snapping or inferred feature
   association.
6. Define add/edit/delete interactions for pointer, touch, and keyboard users
   without breaking CTC-006 two-point measurement. Resolve explicit modes,
   mode switching, focus order, activation, movement/editing, cancel/save,
   selection, destructive confirmation/undo policy, and live announcements.
7. Define carry-arc origin and settings. Resolve selected tee versus target
   origins, how a tee origin is selected when multiple/no tee features exist,
   one or multiple distances, units, min/max/step, defaults, invalid input, and
   per-hole versus per-origin ownership.
8. Define carry-arc geographic math and SVG rendering. Resolve geodesic versus
   local projected circles, latitude behavior, point sampling, closure,
   clipping to the logical inner viewBox, off-map semantics, deterministic
   ordering, labels, color-independent styling, and test tolerances. Do not
   mutate source geometry or imply unreachable precision.
9. Define behavior when a target or arc is outside the selected-hole source
   geometry/view bounds. Resolve whether CTC-006 projection bounds expand to
   include user-authored data, whether imported coordinates must be bounded to
   the selected hole/course bbox, and how invalid/out-of-scope points fail.
10. Define responsive/mobile and accessibility behavior: minimum 44px touch
    targets, focus order, keyboard-only flows, screen-reader names,
    instructions, live regions, error summaries/focus management, destructive
    action semantics, and non-color distinctions.
11. Define exact pure-function, Vitest, and Playwright tests. Include
    deterministic synthetic fixtures, add/edit/delete flows, measurement-mode
    regression, arc math/tolerance, import/export round trip, invalid and
    adversarial files, maximum size, unknown fields, prototype-pollution
    attempts, mismatch handling, mobile behavior, axe scans, and network
    isolation.
12. Define security and privacy behavior. No network upload, HTML injection,
    executable content, dynamic code, unsafe object merging, or trust in
    imported source metadata. State how imported strings are rendered safely.
13. Define observability/debugging. Prefer pure validator results, structured
    error paths/codes, and visible user states over logging side effects.
14. Define exact likely files to add/change and ownership boundaries. Keep the
    implementation small and avoid unrelated refactors.
15. Identify narrow unresolved maintainer decisions, with a recommended
    conservative default for each.

## Decisions that must not be silently invented

- Do not persist normalized hole array indexes or assume hole numbers are
  unique.
- Do not mutate or replace CTC-005 normalized OSM source evidence with imported
  project data.
- Do not trust imported coordinates, IDs, strings, source metadata, or object
  prototypes.
- Do not add snapping, spatial association, target inference, or feature-linked
  targets without explicit justification.
- Do not let target editing silently activate CTC-006 measurement or vice
  versa.
- Do not use visual SVG coordinates as persisted geographic coordinates.
- Do not claim a projected SVG circle is geodesically exact without defining
  and testing the approximation.
- Do not add autosave, IndexedDB, localStorage, service workers, cloud sync,
  accounts, collaboration, providers, geocoding, notes, PDF, or rendered-output
  export.
- Do not hide OSM attribution or normalization warnings while OSM-derived
  geometry is visible.
...
3. Versioned project JSON schema with representative declarations/examples.
4. Strict import validator, error model, safe parsing, and migration policy.
5. Export contract, deterministic serialization, filename/MIME, and source
   attribution/evidence decision.
6. Runtime project-state model and immutable OSM/user-authored separation.
7. Target-marker model, identity, add/edit/delete interactions, and failure
   behavior.
8. Carry-origin/settings model, geographic arc math, projection/rendering,
   clipping, units, and invalid-input behavior.
9. Measurement-mode coexistence, pointer/touch/keyboard behavior, responsive
   layout, and accessibility.
10. Security, privacy, mismatch, warning, error, and observability decisions.
...
1. Defines a strict versioned schema and validator with useful errors.
2. Keeps immutable OSM evidence separate from user-authored project data and
   uses `hole.source.sourceKey` as persisted hole identity.
3. Defines complete target add/edit/delete and carry-arc behavior without
   breaking CTC-006 measurement.
4. Defines deterministic arc math/rendering with bounded test tolerances.
5. Defines safe local-file import/export, accessibility, adversarial tests, and
   network isolation.
6. Keeps CTC-008+, CTC-019, basemaps/providers, PDF/rendered export, external
   user-data flows, and unreviewed persistence out of scope.
...
<!-- BEGIN EXACT FILE 3/18: SECURITY.md -->
...
<!-- END EXACT FILE 3/18: SECURITY.md -->

<!-- BEGIN EXACT FILE 4/18: package.json -->
...
<!-- END EXACT FILE 4/18: package.json -->
...
<!-- BEGIN EXACT FILE 6/18: docs/tile-provider-strategy.md -->
...
<!-- END EXACT FILE 6/18: docs/tile-provider-strategy.md -->

<!-- BEGIN EXACT FILE 7/18: docs/overpass-query-contract.md -->
...
<!-- END EXACT FILE 7/18: docs/overpass-query-contract.md -->

<!-- BEGIN EXACT FILE 8/18: docs/handoffs/ctc-006-gemini-specification-review.md -->
...
<!-- END EXACT FILE 8/18: docs/handoffs/ctc-006-gemini-specification-review.md -->

<!-- BEGIN EXACT FILE 9/18: src/overpass.ts -->
...
<!-- END EXACT FILE 9/18: src/overpass.ts -->

<!-- BEGIN EXACT FILE 10/18: src/normalize.ts -->
...
<!-- END EXACT FILE 10/18: src/normalize.ts -->

<!-- BEGIN EXACT FILE 11/18: src/normalize.test.ts -->
...
<!-- END EXACT FILE 11/18: src/normalize.test.ts -->

<!-- BEGIN EXACT FILE 12/18: src/map.ts -->
...
<!-- END EXACT FILE 12/18: src/map.ts -->

<!-- BEGIN EXACT FILE 13/18: src/map.test.ts -->
...
<!-- END EXACT FILE 13/18: src/map.test.ts -->

<!-- BEGIN EXACT FILE 14/18: src/HoleMap.tsx -->
...
<!-- END EXACT FILE 14/18: src/HoleMap.tsx -->

<!-- BEGIN EXACT FILE 15/18: src/App.tsx -->
...
<!-- END EXACT FILE 15/18: src/App.tsx -->
...
<!-- BEGIN EXACT FILE 18/18: fixtures/overpass/synthetic-golf-course-ctc006.json -->
...
<!-- END EXACT FILE 18/18: fixtures/overpass/synthetic-golf-course-ctc006.json -->
