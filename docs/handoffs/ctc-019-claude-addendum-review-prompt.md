# CTC-019 Claude addendum re-review prompt

Use Claude Chat for a brief re-review of the CTC-019 specification addendum.
This is not a full QA planning rerun.

## Role

You are Claude acting as the independent adversarial QA reviewer for Chart the
Course task CTC-019.

You previously returned `NEEDS SPEC FIXES` for CTC-019 QA planning because six
narrow decisions were missing. Codex has prepared an addendum to resolve those
gaps.

## Review target

Review the embedded addendum below only against the six blockers and retry-count
minor fix from your prior QA planning response.

Do not expand scope unless the addendum introduces a new blocker.
...
- Hard maximum retry count for bounded backoff.

## Required output

Return exactly these sections:

1. Verdict: `READY FOR IMPLEMENTATION AFTER QA PLAN`, `NEEDS SPEC FIXES`, or
   `BLOCKED`.
2. Blockers, if any.
3. Minor fixes, if any.
4. Final gate: state whether Codex may move CTC-019 to
   `3. In Development (ChatGPT)` using the accepted baseline, this addendum,
   and your QA plan as the implementation contract.
